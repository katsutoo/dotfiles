...
3. Confirm the interpreter used by repository commands; do not assume `python` or `python3` resolves correctly. Use the repository's interpreter manager and runner, preferring uv for new projects.
4. Preserve established package and framework choices unless they are unsafe, broken, or clearly block the request. Keep existing environment, formatter, linter, and type-checker tooling during maintenance; a scoped code fix does not authorize a tooling migration.
...
- Preserve the repository's package manager and lockfile policy during maintenance. For a requested migration to uv, reproduce the required environments and deployment artifacts before removing superseded configuration and lockfiles.

## Toolchain And Quality

- For new projects, prefer uv for Python installation, environments, dependencies, locking, building, and execution, Ruff for formatting and linting, and ty for type checking. Use `uv.lock` for applications and development environments that adopt uv.
- Preserve existing tooling and its configured checks unless migration is requested. Do not replace another checker or remove its diagnostics merely to adopt the preferred stack.
- Pin the selected quality tools and environment manager through the repository's dependency groups, lockfile, CI actions, or toolchain metadata. Reproduce the same versions locally and in CI.
- When using Ruff, keep rule selection explicit and version-locked; do not enable every rule blindly. Remove overlapping tools only as part of a requested migration after checking formatting and diagnostic differences.
- When using ty, keep configuration in `[tool.ty]` in `pyproject.toml` and align it with the minimum supported Python. For Python 3.7 through 3.9 targets, account for incomplete bundled standard-library stubs when reviewing diagnostics. Keep suppressions narrow.
- Run the repository's pinned checks with the same invocation locally and in CI. For an Astral toolchain, use `uv run --locked ruff check .`, `uv run --locked ruff format --check .`, and `uv run --locked ty check` when manifest drift must fail.
...
- Use the repository's pinned tools with a target matching `requires-python`. Keep one canonical invocation for local and CI use.
...
- Do not introduce overlapping environment, formatter, linter, or type-checker tooling, or migrate the existing stack as incidental cleanup.
